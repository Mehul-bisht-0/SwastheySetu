...
export default function QueueScreen(): React.ReactNode {
  ...
  if (!state) return <Screen title={t("sync.queued")}><Text style={{ color: ink.muted }}>Loading...</Text></Screen>;
  return (
    <Screen title={t("sync.queued")} footer={state.pending > 0 ? <Button label={t("sync.sendNow")} onPress={onSend} busy={busy} /> : undefined}>
